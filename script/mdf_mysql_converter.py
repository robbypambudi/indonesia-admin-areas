...
def write_insert_body(table_name, rows):
    ...
    for row in rows:
        ...
        if (counter == last_row or counter % SPLIT_ROWS == SPLIT_ROWS - 1):
            print("  ('%s', '%s', '%s');"
                  % (row[0], row[1], escape_string(row[2])))
        else:
            print("  ('%s', '%s', '%s'),"
                  % (row[0], row[1], escape_string(row[2]))
                  )
...
def write_provinces(path):
    ...
    for row in rows:
        ...
        if (counter == last_row or counter % SPLIT_ROWS == SPLIT_ROWS - 1):
            print("  ('%s', '%s');" % (row[0], escape_string(row[1])))
        else:
            print("  ('%s', '%s')," % (row[0], escape_string(row[1]))
                  )
        counter += 1
...
def main(argv):
    if (len(argv) > 0):
        ...
        print("example: python mdf_mysql_converter.py ../data/provinces.csv ../data/regencies.csv ../data/districts.csv ../data/villages.csv")
        sys.exit(2)
